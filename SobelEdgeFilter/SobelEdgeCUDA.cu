...
void gaussianBlurGPU(cv::Mat *inputImage, cv::Mat *outputImage)
{
    ...
    sdkStopTimer(&timer);
    gaussianBlurTime = sdkGetTimerValue(&timer);
...
int main(void)
{
    ...
    input_image = cv::imread("Images\\Input\\win4.jpg", cv::IMREAD_GRAYSCALE);
    output_image = input_image.clone();

    gaussianBlurGPU(&input_image, &output_image);
    sobelGPU(&input_image, &output_image);

    std::cout << std::endl << "Time for Sobel Operator using CUDA (GPU) : " << gaussianBlurTime + sobelFilterTime << " ms" << std::endl;
