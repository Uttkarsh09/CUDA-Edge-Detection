...
void cuda_mem_alloc(void** devPtr, size_t size)
{
...
void cuda_mem_free(void* devPtr)
{
    if (devPtr)
    {
        cudaFree(devPtr);
...
void runCannyOperator(uint8_t *input_image_data, uint8_t *output_image_data, int image_width, int image_height)
{
	...
	cuda_mem_alloc((void**)&input_pixels, sizeof(uint8_t) * image_height * image_width);
	cuda_mem_alloc((void**)&output_pixels, sizeof(uint8_t) * image_height * image_width);
	cuda_mem_alloc((void**)&gradient_pixels, sizeof(double) * image_height * image_width);
	cuda_mem_alloc((void**)&final_result, sizeof(uint8_t) * image_height * image_width);
	cuda_mem_alloc((void**)&max_pixels, sizeof(double) * image_height * image_width);
	cuda_mem_alloc((void**)&segment_pixels, sizeof(uint8_t) * image_height * image_width);
	cuda_mem_alloc((void**)&gaussian_kernel_gpu, sizeof(double) * KERNEL_SIZE * KERNEL_SIZE);
	cuda_mem_alloc((void**)&sobel_kernel_x_gpu, sizeof(int8_t) * 3 * 3);
	cuda_mem_alloc((void**)&sobel_kernel_y_gpu, sizeof(int8_t) * 3 * 3);

	cuda_mem_copy(input_pixels, input_image_data, image_height * image_width * sizeof(uint8_t), cudaMemcpyHostToDevice);
	cuda_mem_copy(gaussian_kernel_gpu, gaussian_kernel, sizeof(double) * KERNEL_SIZE * KERNEL_SIZE, cudaMemcpyHostToDevice);
	cuda_mem_copy(sobel_kernel_x_gpu, sobel_kernel_x, sizeof(int8_t) * KERNEL_SIZE * KERNEL_SIZE, cudaMemcpyHostToDevice);
	cuda_mem_copy(sobel_kernel_y_gpu, sobel_kernel_y, sizeof(int8_t) * KERNEL_SIZE * KERNEL_SIZE, cudaMemcpyHostToDevice);
    ...
    cuda_mem_copy(max_pixels, gradient_pixels, image_height * image_width * sizeof(double), cudaMemcpyDeviceToDevice);
	...
	cuda_mem_copy(final_result, output_pixels, image_height * image_width * sizeof(uint8_t), cudaMemcpyDeviceToDevice);
	...
	cuda_mem_copy(output_image_data, final_result, image_width * image_height * sizeof(uint8_t), cudaMemcpyDeviceToHost);
}
...
void cannyCUDA(int image_number)
{
    switch(image_number)
    {
        case 1:
            ...
            cuda_output_file = "Images\\Output\\Canny-CUDA-1.jpg";
        break;
        case 2:
            cuda_input_file = "Images\\Input\\img2.jpg";
            cuda_output_file = "Images\\Output\\Canny-CUDA-2.jpg";
        break;
        case 3:
            cuda_input_file = "Images\\Input\\img3.jpg";
            cuda_output_file = "Images\\Output\\Canny-CUDA-3.jpg";
        break;
        case 4:
            cuda_input_file = "Images\\Input\\img4.jpg";
            cuda_output_file = "Images\\Output\\Canny-CUDA-4.jpg";
        break;
        case 5:
            cuda_input_file = "Images\\Input\\img5.jpg";
            cuda_output_file = "Images\\Output\\Canny-CUDA-5.jpg";
        break;
        default:
    ...
    runCannyOperator(cuda_input_image.data, cuda_output_image.data, cuda_input_image.cols, cuda_input_image.rows);

    std::cout << std::endl << "Time for Canny Operator using CUDA (GPU) : " << sdkGetTimerValue(&cudaTimer) << " ms" << std::endl;
...
{
	cuda_mem_free(final_result);
    cuda_mem_free(sobel_kernel_y_gpu);
    cuda_mem_free(sobel_kernel_x_gpu);
    cuda_mem_free(gaussian_kernel_gpu);
    cuda_mem_free(segment_pixels);
    cuda_mem_free(max_pixels);
    cuda_mem_free(gradient_pixels);
    cuda_mem_free(output_pixels);
    cuda_mem_free(input_pixels);
